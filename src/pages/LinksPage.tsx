        ...
        <div className="grid grid-cols-2 gap-4">
          {links.map((link, index) => (
            <a
              ...
              className="block"
            >
              <div className="relative bg-gray-800/50 rounded-3xl p-6 aspect-square overflow-hidden group hover:scale-[0.98] transition-transform">
                <div className={`absolute -right-12 -top-12 w-48 h-48 ${link.color} rounded-full opacity-20 group-hover:opacity-30 transition-opacity`}></div>
                <div className="relative z-10 h-full flex flex-col">
                  <div className={`${link.color} text-white w-fit px-4 py-1.5 rounded-full text-sm mb-auto`}>
                    {link.subtitle}
                  </div>
                  <div className="mt-auto">
                    <h2 className="text-2xl font-bold mb-2">{link.title}</h2>
                    <div className={`${link.color} rounded-full p-2 w-fit`}>
                      {link.icon}
                    </div>
                  </div>
                </div>
